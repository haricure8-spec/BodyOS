const Records = (() => {
  ...
  function nextPatientId(){
    return "BP-" + String(nextCounter(PATIENT_COUNTER_KEY)).padStart(6,"0");
  }
  ...
  function findOrCreatePatient(){
    ...
    if(!key){
      return { patientId:"", isNew:false, patientKey:"" };
    }
  ...
  function requiredCheck(){
    ...
    if(before.length && after.length && before.join("・") === after.join("・")){
      warnings.push("施術前後が同じです。入力漏れでなければOK");
    }
  ...
  function checkQuality(showAlert=true){
    ...
    if(el){
      let patientNote = "";
      if(patient){
        patientNote = `<br><span class="quality-ok">継続患者：${patient.patientId}</span>`;
      }else if(patientKey()){
        patientNote = `<br><span class="quality-warn">新規患者として登録予定</span>`;
      }
  ...
  function collect(){
    ...
    if(researchIdInput) researchIdInput.value = researchId;
    ...
    return {
      ...
      visitNumber,
      isNewPatient: patient.isNew,
      createdAt: nowISO(),
      visitDate,

      softwareVersion: SOFTWARE_VERSION,
      bodyosLogicVersion: BODYOS_LOGIC_VERSION,
      ...
      meishiki: {
        ...
        input: result.input,
        detail: {
          lichun: result.detail?.yearP?.lichun || null,
          monthTerm: result.detail?.monthP?.monthInfo || null
        }
      },
      fiveElements: counts,
      prediction: ug.predicted,
      beforeUG: before,
      afterUG: after,
      treatmentMemo: val("treatmentMemo"),
      selfCare: val("homeCare"),
  ...
  function save(){
    const modeOn = $("researchMode")?.checked;
    if(!modeOn){
  ...
  function renderStatus(record, records){
    ...
    if(status){
      status.textContent = `保存件数：${records.length}件 / 患者ID：${record.patientId} / 来院回数：${record.visitNumber}`;
    }
    if(preview){
  ...
  function exportJSON(){
    ...
    download("uglab_research_database.json", JSON.stringify(data, null, 2), "application/json");
  }
  ...
  function exportCSV(){
    const records = getRecords();
    const headers = [
      "patientId","researchId","visitNumber","createdAt","visitDate","softwareVersion","bodyosLogicVersion","qualityScore",
      "patientName","gender","birthYear","birthMonth","birthDay","birthHour","birthMinute",
      "chiefComplaint","yearPillar","monthPillar","dayPillar","hourPillar",
      "wood","fire","earth","metal","water","prediction","beforeUG","afterUG",
      "treatmentMemo","selfCare","nextCheck","outcome","autoSummary"
    ];

    const rows = records.map(r => [
      r.patientId,r.researchId,r.visitNumber,r.createdAt,r.visitDate,r.softwareVersion,r.bodyosLogicVersion,r.dataQuality?.score,
      r.patient?.name,r.patient?.gender,r.patient?.birth?.year,r.patient?.birth?.month,r.patient?.birth?.day,r.patient?.birth?.hour,r.patient?.birth?.minute,
      r.chiefComplaint,r.meishiki?.pillars?.year,r.meishiki?.pillars?.month,r.meishiki?.pillars?.day,r.meishiki?.pillars?.hour,
      r.fiveElements?.["木"],r.fiveElements?.["火"],r.fiveElements?.["土"],r.fiveElements?.["金"],r.fiveElements?.["水"],
      r.prediction,r.beforeUG,r.afterUG,r.treatmentMemo,r.selfCare,r.nextCheck,r.outcome,r.autoCare?.summary
    ]);
