const Reports = (() => {
  ...
  function printMode(mode){
    ...
    setTimeout(()=>{window.print(); document.body.classList.remove(mode);},100);
  }
  function chart(){
    const d=data(), r=d.result, counts=d.counts;
    const before=d.before.join("・")||"-", after=d.after.join("・")||"-";
    document.getElementById("chartReport").innerHTML=`
      <div class="report-a4">
        ...
        <div class="report-grid2">
          <div class="report-card"><h3>五行</h3><div class="report-list">木:${counts["木"]}　火:${counts["火"]}　土:${counts["土"]}　金:${counts["金"]}　水:${counts["水"]}</div></div>
          <div class="report-card"><h3>UG6仮説</h3><div class="report-big">${d.ug.predicted.join("・")}</div><div class="report-list">${d.care.summary}</div></div>
        </div>
        <div class="report-grid2">
          <div class="report-card"><h3>施術前</h3><div class="report-big">${before}</div></div>
          <div class="report-card"><h3>施術後</h3><div class="report-big">${after}</div></div>
        </div>
        <div class="report-grid2">
          <div class="report-card"><h3>施術メモ</h3><div class="report-list">${d.treatment}</div></div>
          <div class="report-card"><h3>セルフケア</h3><div class="report-list">${d.home}</div></div>
        </div>
  ...
  function patient(){
    ...
    document.getElementById("patientReport").innerHTML=`
      <div class="report-a4">
        ...
        <div class="report-grid2">
          <div class="report-card"><h3>今回見つかったこと</h3><div class="report-list">${before.map(c=>"✓ "+BodyOS.found(c)).join("<br>") || "✓ 身体の状態を確認しました"}</div></div>
          <div class="report-card"><h3>施術で変わったこと</h3><div class="report-list">${after.length?after.map(c=>"✓ "+BodyOS.changed(c)+"が変化しました").join("<br>"):"✓ 呼吸・動き・安定感を確認しました"}</div></div>
        </div>
