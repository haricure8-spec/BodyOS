...
function localIP() {
  ...
  for (const list of Object.values(nets)) for (const n of list || []) {
    if (n.family === 'IPv4' && !n.internal && !n.address.startsWith('169.254.')) candidates.push(n.address);
  }
  return candidates.find(x => x.startsWith('192.168.'))
    || candidates.find(x => x.startsWith('10.'))
    || candidates.find(x => /^172\.(1[6-9]|2\d|3[01])\./.test(x))
    || candidates[0]
    || 'localhost';
}
function mime(file) {
...
server.on('error', (error) => {
  ...
  console.error(error && error.code === 'EADDRINUSE' ? `Port ${PORT} is already in use.` : error);
  process.exitCode = 1;
});
